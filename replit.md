...
- **Sistema de autenticação**: Firebase Auth funcionando corretamente com usuários multi-tenant (65 propostas carregadas para usuário Tâmara). Login com Google implementado via OAuth 2.0 authorization code flow (response_type=code). Fluxo: botão "Continuar com Google" → createAuthUri Firebase → Google OAuth → código retorna como query param → servidor troca código por tokens via Google token endpoint (requer GOOGLE_CLIENT_SECRET) → signInWithIdp Firebase com id_token → sessão criada. Secrets necessários: FIREBASE_API_KEY, GOOGLE_CLIENT_SECRET. Domínios OAuth registrados: .repl.co e .replit.dev.
...
- **Atualização Pós-Organização (Março 2026)**: Aprimorado módulo de pós-organização com novos campos no banco de dados: `gratuito` (BOOLEAN) e `hint` (TEXT) em `post_org_templates`, coluna `ordem` (INTEGER) em `post_organization_actions`. Templates reestruturados: Agradecimento (D+1), Acompanhamento (D+7), Ajuste fino (D+30, **gratuito**), Feedback (D+45), Continuidade (D+60). Campo "Ajuste fino" marcado como gratuito com novo texto de saudação e hint estratégico instruindo consultoras a posicionar a visita como cuidado incluído no serviço padrão. Interface atualizada em `pages/pos_organizacao.py`: badge verde "💚 Gratuito" exibido para ações gratuitas, dica estratégica exibida em caixa destacada (ícone 💡, fundo azul) com instruções para consultora. Função `get_template()` retorna 7 valores: emoji, nome, texto, dias_apos, objetivo, gratuito, hint. Templates fallback atualizados com todos os campos. Datas e etapas legadas corrigidas no banco de dados (maiúsculas, nomes antigos). Sistema pronto para expandir modelo de serviços oferta ao cliente (continuidade paga no D+60).
- **Limpeza e Correções (Março 2026)**: (1) Redesenho do bloco de botões em `pos_organizacao.py` com layout [2,1,2] (sugestão|badge|dica), painéis expansíveis, e botão "Usar na observação". (2) Remoção de dependências não usadas (reflex, openai, anthropic, twilio, mercadopago, trafilatura, zipfile36) e duplicatas em `requirements.txt` (41→30 linhas). (3) Correção de variable undefined `acao_id` em pos_organizacao.py (substituído por `acao['id']`). (4) Remoção de import inexistente `utils.pdf_generator_servico_padronizado` em `relatorio_servico_novo.py`. (5) Implementação de `gerar_pdf_venda()` em `pdf_generator_v2.py` com design Navy/Gold para relatório de produtos. (6) Atualização de `gerar_pdf_cliente_proposta()` em `propostas_helper.py` para usar `pdf_generator_v2.gerar_pdf_cliente()` (layout novo em vez de layout antigo). Relatório Cliente e Vendas do Produto agora funcionam com design consistente Navy/Gold.
- **Consolidação Geradores PDF (Março 2026)**: Problema identificado: apenas `relatorio_servico_novo.py` funcionava corretamente; `pdf_generator_v2.py` falhava com "not enough values to unpack" e `pdf_generator_fornecedores.py` com "No module named 'utils.pdf_base'". Solução: consolidado ALL 4 PDF generators (`gerar_pdf_cliente_proposta()`, `gerar_pdf_interno_proposta()`, `gerar_pdf_fornecedores_proposta()`, `gerar_pdf_venda_proposta()`) em `propostas_helper.py` para usar APENAS `relatorio_servico_novo.gerar_pdf_relatorio_servico()`. Todos os relatórios agora usam o mesmo layout profissional Navy/Gold comprovado e funcionando. Testes manuais confirmam: todos os 4 botões geram PDFs corretamente.
