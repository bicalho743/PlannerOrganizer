...
- **Correção Navegação Automática e Autenticação (Agosto 2025)**: Resolvidos problemas críticos de navegação automática aparecendo antes do login e aplicação pulando tela de login. Soluções implementadas: desabilitado `showSidebarNavigation` no config.toml, corrigido erro de importação em gerar_lancamentos_propostas.py, aplicado CSS abrangente para esconder navegação automática, implementada verificação de autenticação robusta que impede acesso ao dashboard sem login, criada interface de login única e integrada diretamente ao Firebase com layout moderno e estatísticas de benefícios.
