...
- **Correção Toggle Sidebar (Agosto 2025)**: Resolvido conflito que impedia o funcionamento da seta de esconder/mostrar sidebar. Removidos estilos CSS conflitantes no .streamlit/style.css que escondiam o botão de toggle (collapsedControl). Desativada correção mobile conflitante em utils/simple_mobile_fix.py. Sidebar agora permite toggle nativo do Streamlit funcionando corretamente. Configurações unificadas em um só local (.streamlit/style.css) para evitar conflitos futuros. Seta de toggle com posição fixa para permanecer sempre visível mesmo quando sidebar está escondida.
