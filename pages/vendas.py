...
def show():
    ...
    # Estado para controlar visibilidade do formulário de nova venda
    if 'vendas_nova_venda_open' not in st.session_state:
        st.session_state.vendas_nova_venda_open = False

    # Botão para abrir/fechar formulário de nova venda
    col_space, col_btn = st.columns([5, 1])
    with col_btn:
        btn_label = "✕ Fechar" if st.session_state.vendas_nova_venda_open else "+ Nova Venda"
        if st.button(btn_label, type="primary", use_container_width=True, key="btn_toggle_nova_venda"):
    ...
    # === Abas principais: Vendas e Análise ===
    tab_lista, tab_analise = st.tabs(["📋 Vendas", "📊 Análise por Período"])
