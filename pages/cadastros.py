...
def show():
    ...
    try:
        # Definir colunas e aliases para cada tipo de cadastro
        if tipo_cadastro == "Cliente":
        ...
        if not registros.empty:
            ...
            # Formulário de ação
            with st.form(f"acao_registro_{tipo_cadastro}"):
                if st.form_submit_button(f"Confirmar {acao}"):
                    ...
                    if acao == "Excluir":
                        try:
                            if tipo_cadastro == "Cliente":
                                st.session_state.db.delete_cliente(registro_id)
                            elif tipo_cadastro == "Fornecedor":
            ...
            # Formulário de edição (moved here from inside the expander)
                ...
                with st.form(f"edit_form_{registro_id}"):
                    ...
                    with col1:
                        if st.form_submit_button("Salvar"):
                            try:
                                if tipo_cadastro == "Cliente":
                                    st.session_state.db.update_cliente(registro_id, **edited_data)
                                elif tipo_cadastro == "Fornecedor":
