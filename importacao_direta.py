...
st.write("Este script irá importar as propostas diretamente da planilha fornecida.")
...
# Mostrar dataframe
st.write("### Propostas a serem importadas:")
...
def importar_propostas_direto():
    ...
    if st.checkbox("Mostrar lista de clientes disponíveis"):
        st.write("### Clientes disponíveis no sistema:")
        clientes_df = client_mappings['all_clients'][['id', 'nome']]
...
if st.button("⚡ Iniciar Importação Direta", type="primary"):
    with st.spinner("Importando propostas..."):
        sucesso, mensagem = importar_propostas_direto()
        if sucesso:
            st.success(mensagem)
        else:
            st.error(mensagem)