    ...
    try:
        ...
        try:
                ...
                for sep in separadores:
                    try:
                        ...
                        if df.shape[0] > 0 and df.shape[1] > 1:
                            st.success(
                                f"Arquivo lido com sucesso usando separador '{sep}' e codificação '{encoding}'"
                            )
                            break
                    except Exception as e:
                        continue
            ...
            if df is None or df.empty:
                return (
                    False,
                    "Erro ao ler arquivo: arquivo vazio ou formato não suportado",
                )
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if "usuario_id" in st.session_state and st.session_state.usuario_id:
                    ...
                    print(
                        f"DEBUG MULTI-TENANT: Atribuindo usuario_id={st.session_state.usuario_id} ao cliente {nome}"
                    )
                ...
                sucessos += 1
                st.write(
                    f"✅ Cliente '{nome}' (ID: {cliente_id}) importado com sucesso"
                )
        ...
        if sucessos > 0:
            mensagem = (
                f"Importação concluída: {sucessos} clientes importados com sucesso!"
            )
            if erros:
    ...
    try:
        ...
        if st.button("Iniciar Importação", type="primary"):
            with st.spinner("Importando clientes..."):
                sucesso, mensagem = importar_clientes_com_id(
                    arquivo, st.session_state.db
                )
                if sucesso:
                    st.success(mensagem)
