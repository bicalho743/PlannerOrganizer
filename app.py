...
if 'page' in query_params and query_params['page'][0] == 'planos':
    ...
    if usuario_id and not tem_assinatura:
        ...
        if st.button("Fazer Login"):
            st.markdown("""
            <script>
                window.location.href = "/?redirect=/planos";
            </script>
            """, unsafe_allow_html=True)
    elif tem_assinatura:
    ...
    # Parar a execução após mostrar a página de planos
    st.stop()

elif 'page' in query_params and query_params['page'][0] == 'iniciar_teste':
    # Implementação direta da página de teste gratuito
    try:
        ...
        st.error(f"Não foi possível carregar a página de início de teste: {str(e)}")
        st.markdown("Voltando para a página principal em 3 segundos...")
        st.markdown("""
        <script>
            setTimeout(function() {
