...
if is_render:
    logger.info(
        "Ambiente Render detectado, executando scripts de inicialização...")
    try:
        ...
        logger.error(
            f"Erro ao executar script de inicialização do Render: {str(e)}")
...
try:
    ...
    if success:
        ...
        logger.warning(
            "Não foi possível registrar adaptadores para numpy.int*")
except Exception as e:
...
try:
    ...
    firebase_auth = None
    st.warning(
        "Módulo Firebase Auth não encontrado. Usando autenticação padrão.")
...
st.set_page_config(page_title="Planner Organizer - Sistema Profissional",
...
try:
    if firebase_auth is not None:
        ...
        logger.warning(
            "⚠️ Firebase Auth: INATIVO - Usando autenticação padrão")
except Exception as e:
...
def debug_propostas_finalizadas():
    ...
    st.set_page_config(page_title="Propostas Finalizadas",
                       page_icon="🔍",
                       layout="wide")
    ...
    st.warning(
        "Esta é uma ferramenta de diagnóstico para resolver o problema de filtragem de propostas finalizadas."
    )
    ...
    st.info(
        f"Total de propostas no banco de dados: {len(todas_propostas) if not todas_propostas.empty else 0}"
    )
    ...
    if not todas_propostas.empty:
        ...
        colunas_display = [
            'id', 'numero', 'cliente_nome', 'status', 'status_execucao',
            'valor'
        ]
        ...
        for idx, p in todas_propostas.iterrows():
            ...
            if (status_match and exec_match) or recusada_match:
                st.success(
                    f"✅ PROPOSTA #{p['numero']} - {p['cliente_nome']} ATENDE AOS CRITÉRIOS"
                )
                st.write(f"- Status: {p['status']}")
                st.write(f"- Status Execução: {p['status_execucao']}")
            else:
                st.error(
                    f"❌ PROPOSTA #{p['numero']} - {p['cliente_nome']} NÃO ATENDE AOS CRITÉRIOS"
                )
                st.write(f"- Status: {p['status']}")
                st.write(f"- Status Execução: {p['status_execucao']}")
            st.write("---")
    ...
    if not todas_propostas.empty:
        ...
        st.write(
            f"Total de propostas após filtragem: {len(propostas_finalizadas)}")
        ...
        if not propostas_finalizadas.empty:
            ...
            for idx, proposta in propostas_finalizadas.iterrows():
                with st.expander(
                        f"{proposta['numero']} - {proposta['cliente_nome']} - {proposta['descricao']} (R$ {proposta['valor']:.2f})"
                ):
                    ...
                    with col2:
                        ...
                        st.write(
                            f"**Status Execução:** {proposta['status_execucao']}"
                        )
                        data_inicio_str = proposta['data_inicio'].strftime(
                            '%d/%m/%Y') if pd.notna(
                                proposta['data_inicio']) else 'N/D'
                        st.write(f"**Data Início:** {data_inicio_str}")
                        data_fim_str = proposta['data_fim'].strftime(
                            '%d/%m/%Y') if pd.notna(
                                proposta['data_fim']) else 'N/D'
    ...
    st.code(
        f"- Database URL: {'Usando variável de ambiente' if 'DATABASE_URL' in os.environ else 'Não configurada'}"
    )
    st.code(
        f"- Hora da consulta: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if 'show_enviar_manual' in query_params and query_params[
        'show_enviar_manual'] == 'true':
...
if 'planos_standalone_page' in st.query_params:
    ...
    try:
        ...
        st.error(
            f"Não foi possível carregar a página standalone de planos: {e}")
...
if not st.session_state.authenticated:
    ...
    elif "show_politica" in query_params and query_params[
            "show_politica"] == "true":
        try:
            from pages.politica_privacidade import show
            show()
            ...
            st.error(
                f"Não foi possível carregar a política de privacidade: {e}")
    ...
    if st.session_state.login_page == "registrar":
        ...
        try:
            ...
            st.error(
                f"Não foi possível carregar o módulo de recuperação de senha: {e}"
            )
            # Resetar para página de login
            st.session_state.login_page = "login"
            st.rerun()
    ...
    with left_col:
        # Cabeçalho principal
        st.markdown('<h1 class="main-header">Planner Organizer</h1>',
                    unsafe_allow_html=True)
        st.markdown(
            '<p class="subheader">Sistema Profissional para Personal Organizers</p>',
            unsafe_allow_html=True)
        ...
        st.markdown("<h3>Por que escolher o Planner Organizer?</h3>",
                    unsafe_allow_html=True)
        ...
        st.markdown("<h3>O que nossos clientes dizem</h3>",
                    unsafe_allow_html=True)
        ...
        # Imagem promocional
        st.markdown("<h2>Transforme Seu Negócio com o Planner Organizer</h2>",
                    unsafe_allow_html=True)
        ...
        with faq_container:
            # Pergunta 1
            with st.expander(
                    "Como o sistema ajuda a manter contato com as clientes?"):
                st.markdown("""
            ...
            # Pergunta 4
            with st.expander(
                    "O sistema guarda histórico de atendimentos às clientes?"):
                st.markdown("""
        ...
        with testimonial_cols[0]:
            st.markdown("**Mônica Alves**<br>*Personal Organizer*",
                        unsafe_allow_html=True)
        with testimonial_cols[1]:
            st.markdown("**Ana Prata**<br>*Personal Organizer*",
                        unsafe_allow_html=True)
        with testimonial_cols[2]:
            st.markdown("**Lívia Martins**<br>*Personal Organizer*",
                        unsafe_allow_html=True)
        with testimonial_cols[3]:
            st.markdown("**Isabela Silva**<br>*Personal Organizer*",
                        unsafe_allow_html=True)
        with testimonial_cols[4]:
            st.markdown("**Mariana Costa**<br>*Personal Organizer*",
                        unsafe_allow_html=True)
    ...
    with right_col:
        ...
        with st.form("login_form"):
            ...
            st.markdown('<div style="height: 15px;"></div>',
                        unsafe_allow_html=True)
            submit = st.form_submit_button("Entrar na minha conta",
                ...
                if firebase_auth is not None:
                    with st.spinner("Autenticando..."):
                        ...
                        if result['success']:
                            ...
                            st.error(
                                f"Erro de autenticação: {result['error']}")
                else:
                    st.error(
    ...
    try:
        ...
        # Verificar st.session_state.usuario (alternativa)
            ...
            if isinstance(st.session_state.usuario,
                          dict) and 'id' in st.session_state.usuario:
                usuario_id = st.session_state.usuario['id']
                print(
                    f"DEBUG MULTI-TENANT: Encontrado usuario_id={usuario_id} no objeto usuario"
                )

        # Inicializar o Database com o ID do usuário quando disponível
        if usuario_id:
            print(
                f"DEBUG MULTI-TENANT: Inicializando Database com usuario_id={usuario_id}"
            )
            st.session_state.db = Database(usuario_id=usuario_id)
        else:
            print(
                "DEBUG MULTI-TENANT: AVISO - Inicializando Database sem ID de usuário!"
            )
        ...
        st.warning(
            "Se você estiver usando Neon PostgreSQL ou outro banco de dados serverless, você precisa reativar o endpoint."
        )
        ...
        if db_url:
            ...
            if len(safe_url) > 1:
                ...
                st.info(
                    f"Sua conexão de banco de dados aponta para: ...@{host_part}"
                )
            else:
                st.info(
                    "DATABASE_URL está definido, mas não está no formato esperado."
                )
        else:
...
if st.session_state.get('autenticado', False) and getattr(
        st.session_state.get('usuario', None), 'tipo', '') == 'admin':
...
apply_page_header()
apply_page_footer()
# Carregue o CSS global
with open(".streamlit/style.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

# No final da renderização da página, aplique o rodapé
...
st.sidebar.markdown(
    '<div style="margin: 1.5rem 0;"><hr style="border: none; height: 1px; background-color: #E0E0E0;"></div>',
    unsafe_allow_html=True)
...
with st.sidebar.expander("ℹ️ Informações do Sistema"):
    ...
    if st.button("📘 Gerar Manual do Sistema", use_container_width=True):
        with st.spinner("Gerando manual em PDF..."):
            try:
                ...
                st.download_button(label="📥 Baixar Manual do Sistema",
                                   data=pdf_bytes,
    ...
    try:
        ...
        st.download_button(
            ...
            use_container_width=True,
            help=
            "Baixe todos os ícones do sistema em diferentes formatos (SVG, PNG, Favicon)"
        )
    except Exception as e:
